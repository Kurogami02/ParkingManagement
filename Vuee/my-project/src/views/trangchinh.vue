<template>
  ...
  <v-dialog v-model="showForm" max-width="600px">
    <v-card style="padding: 15px;">
      ...
      <v-text-field label="Tên đại lý" v-model="TenXe"></v-text-field>
      <v-text-field label="Địa chi" v-model="ViTri"></v-text-field>
    ...
    <v-dialog v-model="editDialog" max-width="600px">
      <v-card>
        ...
        <v-card-text>
          <v-text-field v-model="editedBaiXe.TenXe" label="Tên Đại Lý"></v-text-field>
          <v-text-field v-model="editedBaiXe.ViTri" label="Địa Chỉ"></v-text-field>
...
export default {
  ...
  methods: {
    ...
    confirmDelete(ID) {
      if (confirm("Bạn có muốn xóa Đại lý có id: " + ID + " không?")) {
        this.deleteBaiXe(ID);
      }
    },
    deleteBaiXe(ID) {
