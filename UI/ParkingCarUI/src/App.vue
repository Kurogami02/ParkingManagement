<template>
  <v-app id="inspire" style="padding: 15px;">
    <v-navigation-drawer v-model="drawer" color="#aac6da">
      <template v-slot:prepend>
        <v-list-item
          lines="two"
          prepend-avatar="C:\Users\1011s\Downloadsimg.jpg"
          title="Quốc Khánh"
          subtitle=""
        ></v-list-item>
      </template>
      ...
      <v-list density="compact" nav>
        <v-list-item @click="navigateTo('home')" prepend-icon="mdi-home-city" title="Trang chủ"></v-list-item>
        <v-list-item @click="navigateTo('dashboard')" prepend-icon="mdi-view-dashboard" title="Quản lý"></v-list-item>
        <v-list-item @click="navigateTo('car')" prepend-icon="mdi-folder" title="Kiểm kê"></v-list-item>
        <v-list-item @click="navigateTo('infor')" prepend-icon="mdi-account-group-outline" title="Thống kê"></v-list-item>
        
      </v-list>
    </v-navigation-drawer>

    <v-app-bar class="custom-app-bar" color="#5ba3d7">
    <v-app-bar-nav-icon @click="checkLogin()"></v-app-bar-nav-icon>
    <v-app-bar-title>Quản lý đại lý bán vé máy bay</v-app-bar-title>
...
<v-dialog v-model="showLoginFormDialog" max-width="400">
  <!-- Dialog đăng nhập -->
    ...
    <v-footer class="" style="background-color: #aac6da;">
